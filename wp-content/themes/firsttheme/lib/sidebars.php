...
function _themename_sidebar_widgets()
{
    register_sidebar(array(
        ...
        'before_widget' => '<section id="%1s" class="c-sidebar-widget u-margin-bottom-20 %2$s">',
...
foreach ($columns as $i => $column) {
    register_sidebar(array(
        ...
        'name' => sprintf(esc_html__('Footer Widgets Column %', '_themename'), $i + 1),
        'description' => esc_html__('Footer widgets', '_themename'),
        'before_widget' => '<section id="%1s" class="c-footer-widget %2$s' . $widget_theme . '%22$s">',
        'after_widget' => '</section>',
        'before_title' => '<h5>',
        'after_title' => '</h5>'
    ));
